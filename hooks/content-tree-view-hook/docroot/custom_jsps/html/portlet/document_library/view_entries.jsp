...
<%@ include file="/html/portlet/document_library/view_entries_tree.jspf" %>
						...
						<c:when test="<%= DLFileEntryPermission.contains(permissionChecker, fileEntry, ActionKeys.VIEW) %>">
							...
							<c:choose>
								...
								<c:when test='<%= displayStyle.equals(TREE_VIEW) %>'>
									<liferay-util:include page="/html/portlet/document_library/view_file_entry_tree.jsp" />
								</c:when>
								<%-- RivetLogic Custom ENDS --%>
								<c:otherwise>
			...
			<c:choose>
				<c:when test='<%= !displayStyle.equals("list") %>'>
					...
					<c:choose>
						...
						<%-- RivetLogic Custom BEGINS --%>
						<c:when test='<%= displayStyle.equals(TREE_VIEW) %>'>
							<liferay-util:include page="/html/portlet/document_library/view_folder_tree.jsp" />
						</c:when>
						<%-- RivetLogic Custom ENDS --%>
						<c:otherwise>
